...
import type { Customer, Product, Order } from "./Orders.types";
...
// چون آیتم‌های ذخیره‌شده سفارش فقط id محصول/ویژگی رو دارن، اسمشون رو از روی لیست محصولات پیدا می‌کنیم
// تا توی سبد خرید مثل صفحه ثبت سفارش نمایش داده بشن
const findProductName = (productId: number) =>
    props.products.find((p) => p.id === productId)?.name ?? "محصول حذف‌شده";
...
const findAttributeName = (
    ...
    return attr ? `${attr.key}: ${attr.value} ${attr.unit || ""}` : "";
};

// فرم ویرایش سفارش با مقداردهی اولیه از روی سفارش فعلی
const form = useForm({
    customer_id: props.order.customer_id as number | "",
    delivery_date: props.order.delivery_date,
    payment_type: props.order.payment_type,
    installment_date: props.order.installment_date ?? "",
    ...
    items: props.order.items.map((item) => ({
        ...
        product_name: findProductName(item.product_id),
        attribute_name: findAttributeName(
            item.product_id,
...
// ارسال ویرایش‌های فرم به سمت بک‌اند
const submitOrder = () => {
...
<template>
    ...
    <MainLayout>
        <div class="mb-6 mt-2">
            <h2 class="text-2xl font-black text-slate-800">ویرایش سفارش ✏️</h2>
            <p class="text-slate-500 text-sm mt-1">
                اطلاعات فاکتور رو اصلاح کن و ذخیره کن
            </p>
        </div>
        ...
        <form @submit.prevent="submitOrder" class="space-y-6 pb-10">
            ...
            <div class="space-y-3">
                ...
                <button
                    ...
                    @click="deleteOrder"
                    :disabled="isDeleting"
                    class="w-full bg-rose-50 disabled:opacity-50 text-rose-600 py-3.5 rounded-2xl text-sm font-bold transition-transform active:scale-95 flex justify-center items-center"
                >
                    حذف کامل این سفارش
                </button>
            </div>
